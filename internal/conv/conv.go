...
// Selection values, which 4.7 returns as {value,label} objects, are unwrapped.
func CustomFieldsFromAPI(m map[string]any, prior jsontypes.Normalized) jsontypes.Normalized {
	if !Known(prior) {
		// Not configured: track nothing (null) so the attribute stays null.
		if prior.IsNull() {
			return jsontypes.NewNormalizedNull()
		}
	}
	...
	if Known(prior) {
		...
		if err := json.Unmarshal([]byte(prior.ValueString()), &cfg); err == nil {
	...
	for k, v := range m {
		...
		out[k] = unwrapChoice(v)
	}
	if keep == nil && len(out) == 0 {
		return jsontypes.NewNormalizedNull()
	}
	return JSONFromAPI(out)
}

// AllCustomFieldsFromAPI returns every custom field (data sources).
func AllCustomFieldsFromAPI(m map[string]any) jsontypes.Normalized {
	if len(m) == 0 {
		return jsontypes.NewNormalizedNull()
	}
	out := map[string]any{}
...
func unwrapChoice(v any) any {
	switch t := v.(type) {
	case map[string]any:
		if val, ok := t["value"]; ok && len(t) == 2 {
			if _, hasLabel := t["label"]; hasLabel {
				return val
			}
		}
		return t
