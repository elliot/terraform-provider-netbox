...
// writeIfGenerated writes an example unless the directory is hand-maintained.
// Generated example directories contain a ".generated" marker file; delete it
// to take over the examples by hand.
func writeIfGenerated(path, content string) error {
	dir := filepath.Dir(path)
	marker := filepath.Join(dir, ".generated")
	if _, err := os.Stat(marker); err != nil {
		if _, err := os.Stat(path); err == nil {
			return nil // hand-maintained
		}
		if err := os.WriteFile(marker, []byte("Files in this directory are generated by internal/gen. Delete this marker to hand-maintain them.\n"), 0o600); err != nil {
			return err
		}
	}
