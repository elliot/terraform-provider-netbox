// Code generated by openapi-generator from the NetBox 4.7.0 OpenAPI document. DO NOT EDIT.

/*
NetBox REST API

No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

API version: 4.7.0 (4.7)
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package netbox

import (
	"encoding/json"
	"fmt"
	"time"
)

// checks if the CoolingFeed type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &CoolingFeed{}

// CoolingFeed Base serializer class for models inheriting from PrimaryModel.
type CoolingFeed struct {
	Id            int32               `json:"id"`
	Url           *string             `json:"url,omitempty"`
	DisplayUrl    *string             `json:"display_url,omitempty"`
	Display       *string             `json:"display,omitempty"`
	CoolingSource *BriefCoolingSource `json:"cooling_source,omitempty"`
	Rack          NullableBriefRack   `json:"rack,omitempty"`
	Name          *string             `json:"name,omitempty"`
	Status        *ChoiceString       `json:"status,omitempty"`
	// Rated cooling capacity (kW)
	CoolingCapacity      NullableFloat64        `json:"cooling_capacity,omitempty"`
	MaxFlow              NullableFloat64        `json:"max_flow,omitempty"`
	MaxFlowUnit          NullableChoiceString   `json:"max_flow_unit,omitempty"`
	Description          *string                `json:"description,omitempty"`
	Tenant               NullableBriefTenant    `json:"tenant,omitempty"`
	Owner                NullableBriefOwner     `json:"owner,omitempty"`
	Comments             *string                `json:"comments,omitempty"`
	Tags                 []NestedTag            `json:"tags,omitempty"`
	CustomFields         map[string]interface{} `json:"custom_fields,omitempty"`
	Created              NullableTime           `json:"created,omitempty"`
	LastUpdated          NullableTime           `json:"last_updated,omitempty"`
	AdditionalProperties map[string]interface{}
}

type _CoolingFeed CoolingFeed

// NewCoolingFeed instantiates a new CoolingFeed object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewCoolingFeed(id int32) *CoolingFeed {
	this := CoolingFeed{}
	this.Id = id
	return &this
}

// NewCoolingFeedWithDefaults instantiates a new CoolingFeed object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewCoolingFeedWithDefaults() *CoolingFeed {
	this := CoolingFeed{}
	return &this
}

// GetId returns the Id field value
func (o *CoolingFeed) GetId() int32 {
	if o == nil {
		var ret int32
		return ret
	}

	return o.Id
}

// GetIdOk returns a tuple with the Id field value
// and a boolean to check if the value has been set.
func (o *CoolingFeed) GetIdOk() (*int32, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Id, true
}

// SetId sets field value
func (o *CoolingFeed) SetId(v int32) {
	o.Id = v
}

// GetUrl returns the Url field value if set, zero value otherwise.
func (o *CoolingFeed) GetUrl() string {
	if o == nil || IsNil(o.Url) {
		var ret string
		return ret
	}
	return *o.Url
}

// GetUrlOk returns a tuple with the Url field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CoolingFeed) GetUrlOk() (*string, bool) {
	if o == nil || IsNil(o.Url) {
		return nil, false
	}
	return o.Url, true
}

// HasUrl returns a boolean if a field has been set.
func (o *CoolingFeed) HasUrl() bool {
	if o != nil && !IsNil(o.Url) {
		return true
	}

	return false
}

// SetUrl gets a reference to the given string and assigns it to the Url field.
func (o *CoolingFeed) SetUrl(v string) {
	o.Url = &v
}

// GetDisplayUrl returns the DisplayUrl field value if set, zero value otherwise.
func (o *CoolingFeed) GetDisplayUrl() string {
	if o == nil || IsNil(o.DisplayUrl) {
		var ret string
		return ret
	}
	return *o.DisplayUrl
}

// GetDisplayUrlOk returns a tuple with the DisplayUrl field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CoolingFeed) GetDisplayUrlOk() (*string, bool) {
	if o == nil || IsNil(o.DisplayUrl) {
		return nil, false
	}
	return o.DisplayUrl, true
}

// HasDisplayUrl returns a boolean if a field has been set.
func (o *CoolingFeed) HasDisplayUrl() bool {
	if o != nil && !IsNil(o.DisplayUrl) {
		return true
	}

	return false
}

// SetDisplayUrl gets a reference to the given string and assigns it to the DisplayUrl field.
func (o *CoolingFeed) SetDisplayUrl(v string) {
	o.DisplayUrl = &v
}

// GetDisplay returns the Display field value if set, zero value otherwise.
func (o *CoolingFeed) GetDisplay() string {
	if o == nil || IsNil(o.Display) {
		var ret string
		return ret
	}
	return *o.Display
}

// GetDisplayOk returns a tuple with the Display field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CoolingFeed) GetDisplayOk() (*string, bool) {
	if o == nil || IsNil(o.Display) {
		return nil, false
	}
	return o.Display, true
}

// HasDisplay returns a boolean if a field has been set.
func (o *CoolingFeed) HasDisplay() bool {
	if o != nil && !IsNil(o.Display) {
		return true
	}

	return false
}

// SetDisplay gets a reference to the given string and assigns it to the Display field.
func (o *CoolingFeed) SetDisplay(v string) {
	o.Display = &v
}

// GetCoolingSource returns the CoolingSource field value if set, zero value otherwise.
func (o *CoolingFeed) GetCoolingSource() BriefCoolingSource {
	if o == nil || IsNil(o.CoolingSource) {
		var ret BriefCoolingSource
		return ret
	}
	return *o.CoolingSource
}

// GetCoolingSourceOk returns a tuple with the CoolingSource field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CoolingFeed) GetCoolingSourceOk() (*BriefCoolingSource, bool) {
	if o == nil || IsNil(o.CoolingSource) {
		return nil, false
	}
	return o.CoolingSource, true
}

// HasCoolingSource returns a boolean if a field has been set.
func (o *CoolingFeed) HasCoolingSource() bool {
	if o != nil && !IsNil(o.CoolingSource) {
		return true
	}

	return false
}

// SetCoolingSource gets a reference to the given BriefCoolingSource and assigns it to the CoolingSource field.
func (o *CoolingFeed) SetCoolingSource(v BriefCoolingSource) {
	o.CoolingSource = &v
}

// GetRack returns the Rack field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *CoolingFeed) GetRack() BriefRack {
	if o == nil || IsNil(o.Rack.Get()) {
		var ret BriefRack
		return ret
	}
	return *o.Rack.Get()
}

// GetRackOk returns a tuple with the Rack field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *CoolingFeed) GetRackOk() (*BriefRack, bool) {
	if o == nil {
		return nil, false
	}
	return o.Rack.Get(), o.Rack.IsSet()
}

// HasRack returns a boolean if a field has been set.
func (o *CoolingFeed) HasRack() bool {
	if o != nil && o.Rack.IsSet() {
		return true
	}

	return false
}

// SetRack gets a reference to the given NullableBriefRack and assigns it to the Rack field.
func (o *CoolingFeed) SetRack(v BriefRack) {
	o.Rack.Set(&v)
}

// SetRackNil sets the value for Rack to be an explicit nil
func (o *CoolingFeed) SetRackNil() {
	o.Rack.Set(nil)
}

// UnsetRack ensures that no value is present for Rack, not even an explicit nil
func (o *CoolingFeed) UnsetRack() {
	o.Rack.Unset()
}

// GetName returns the Name field value if set, zero value otherwise.
func (o *CoolingFeed) GetName() string {
	if o == nil || IsNil(o.Name) {
		var ret string
		return ret
	}
	return *o.Name
}

// GetNameOk returns a tuple with the Name field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CoolingFeed) GetNameOk() (*string, bool) {
	if o == nil || IsNil(o.Name) {
		return nil, false
	}
	return o.Name, true
}

// HasName returns a boolean if a field has been set.
func (o *CoolingFeed) HasName() bool {
	if o != nil && !IsNil(o.Name) {
		return true
	}

	return false
}

// SetName gets a reference to the given string and assigns it to the Name field.
func (o *CoolingFeed) SetName(v string) {
	o.Name = &v
}

// GetStatus returns the Status field value if set, zero value otherwise.
func (o *CoolingFeed) GetStatus() ChoiceString {
	if o == nil || IsNil(o.Status) {
		var ret ChoiceString
		return ret
	}
	return *o.Status
}

// GetStatusOk returns a tuple with the Status field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CoolingFeed) GetStatusOk() (*ChoiceString, bool) {
	if o == nil || IsNil(o.Status) {
		return nil, false
	}
	return o.Status, true
}

// HasStatus returns a boolean if a field has been set.
func (o *CoolingFeed) HasStatus() bool {
	if o != nil && !IsNil(o.Status) {
		return true
	}

	return false
}

// SetStatus gets a reference to the given ChoiceString and assigns it to the Status field.
func (o *CoolingFeed) SetStatus(v ChoiceString) {
	o.Status = &v
}

// GetCoolingCapacity returns the CoolingCapacity field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *CoolingFeed) GetCoolingCapacity() float64 {
	if o == nil || IsNil(o.CoolingCapacity.Get()) {
		var ret float64
		return ret
	}
	return *o.CoolingCapacity.Get()
}

// GetCoolingCapacityOk returns a tuple with the CoolingCapacity field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *CoolingFeed) GetCoolingCapacityOk() (*float64, bool) {
	if o == nil {
		return nil, false
	}
	return o.CoolingCapacity.Get(), o.CoolingCapacity.IsSet()
}

// HasCoolingCapacity returns a boolean if a field has been set.
func (o *CoolingFeed) HasCoolingCapacity() bool {
	if o != nil && o.CoolingCapacity.IsSet() {
		return true
	}

	return false
}

// SetCoolingCapacity gets a reference to the given NullableFloat64 and assigns it to the CoolingCapacity field.
func (o *CoolingFeed) SetCoolingCapacity(v float64) {
	o.CoolingCapacity.Set(&v)
}

// SetCoolingCapacityNil sets the value for CoolingCapacity to be an explicit nil
func (o *CoolingFeed) SetCoolingCapacityNil() {
	o.CoolingCapacity.Set(nil)
}

// UnsetCoolingCapacity ensures that no value is present for CoolingCapacity, not even an explicit nil
func (o *CoolingFeed) UnsetCoolingCapacity() {
	o.CoolingCapacity.Unset()
}

// GetMaxFlow returns the MaxFlow field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *CoolingFeed) GetMaxFlow() float64 {
	if o == nil || IsNil(o.MaxFlow.Get()) {
		var ret float64
		return ret
	}
	return *o.MaxFlow.Get()
}

// GetMaxFlowOk returns a tuple with the MaxFlow field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *CoolingFeed) GetMaxFlowOk() (*float64, bool) {
	if o == nil {
		return nil, false
	}
	return o.MaxFlow.Get(), o.MaxFlow.IsSet()
}

// HasMaxFlow returns a boolean if a field has been set.
func (o *CoolingFeed) HasMaxFlow() bool {
	if o != nil && o.MaxFlow.IsSet() {
		return true
	}

	return false
}

// SetMaxFlow gets a reference to the given NullableFloat64 and assigns it to the MaxFlow field.
func (o *CoolingFeed) SetMaxFlow(v float64) {
	o.MaxFlow.Set(&v)
}

// SetMaxFlowNil sets the value for MaxFlow to be an explicit nil
func (o *CoolingFeed) SetMaxFlowNil() {
	o.MaxFlow.Set(nil)
}

// UnsetMaxFlow ensures that no value is present for MaxFlow, not even an explicit nil
func (o *CoolingFeed) UnsetMaxFlow() {
	o.MaxFlow.Unset()
}

// GetMaxFlowUnit returns the MaxFlowUnit field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *CoolingFeed) GetMaxFlowUnit() ChoiceString {
	if o == nil || IsNil(o.MaxFlowUnit.Get()) {
		var ret ChoiceString
		return ret
	}
	return *o.MaxFlowUnit.Get()
}

// GetMaxFlowUnitOk returns a tuple with the MaxFlowUnit field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *CoolingFeed) GetMaxFlowUnitOk() (*ChoiceString, bool) {
	if o == nil {
		return nil, false
	}
	return o.MaxFlowUnit.Get(), o.MaxFlowUnit.IsSet()
}

// HasMaxFlowUnit returns a boolean if a field has been set.
func (o *CoolingFeed) HasMaxFlowUnit() bool {
	if o != nil && o.MaxFlowUnit.IsSet() {
		return true
	}

	return false
}

// SetMaxFlowUnit gets a reference to the given NullableChoiceString and assigns it to the MaxFlowUnit field.
func (o *CoolingFeed) SetMaxFlowUnit(v ChoiceString) {
	o.MaxFlowUnit.Set(&v)
}

// SetMaxFlowUnitNil sets the value for MaxFlowUnit to be an explicit nil
func (o *CoolingFeed) SetMaxFlowUnitNil() {
	o.MaxFlowUnit.Set(nil)
}

// UnsetMaxFlowUnit ensures that no value is present for MaxFlowUnit, not even an explicit nil
func (o *CoolingFeed) UnsetMaxFlowUnit() {
	o.MaxFlowUnit.Unset()
}

// GetDescription returns the Description field value if set, zero value otherwise.
func (o *CoolingFeed) GetDescription() string {
	if o == nil || IsNil(o.Description) {
		var ret string
		return ret
	}
	return *o.Description
}

// GetDescriptionOk returns a tuple with the Description field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CoolingFeed) GetDescriptionOk() (*string, bool) {
	if o == nil || IsNil(o.Description) {
		return nil, false
	}
	return o.Description, true
}

// HasDescription returns a boolean if a field has been set.
func (o *CoolingFeed) HasDescription() bool {
	if o != nil && !IsNil(o.Description) {
		return true
	}

	return false
}

// SetDescription gets a reference to the given string and assigns it to the Description field.
func (o *CoolingFeed) SetDescription(v string) {
	o.Description = &v
}

// GetTenant returns the Tenant field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *CoolingFeed) GetTenant() BriefTenant {
	if o == nil || IsNil(o.Tenant.Get()) {
		var ret BriefTenant
		return ret
	}
	return *o.Tenant.Get()
}

// GetTenantOk returns a tuple with the Tenant field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *CoolingFeed) GetTenantOk() (*BriefTenant, bool) {
	if o == nil {
		return nil, false
	}
	return o.Tenant.Get(), o.Tenant.IsSet()
}

// HasTenant returns a boolean if a field has been set.
func (o *CoolingFeed) HasTenant() bool {
	if o != nil && o.Tenant.IsSet() {
		return true
	}

	return false
}

// SetTenant gets a reference to the given NullableBriefTenant and assigns it to the Tenant field.
func (o *CoolingFeed) SetTenant(v BriefTenant) {
	o.Tenant.Set(&v)
}

// SetTenantNil sets the value for Tenant to be an explicit nil
func (o *CoolingFeed) SetTenantNil() {
	o.Tenant.Set(nil)
}

// UnsetTenant ensures that no value is present for Tenant, not even an explicit nil
func (o *CoolingFeed) UnsetTenant() {
	o.Tenant.Unset()
}

// GetOwner returns the Owner field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *CoolingFeed) GetOwner() BriefOwner {
	if o == nil || IsNil(o.Owner.Get()) {
		var ret BriefOwner
		return ret
	}
	return *o.Owner.Get()
}

// GetOwnerOk returns a tuple with the Owner field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *CoolingFeed) GetOwnerOk() (*BriefOwner, bool) {
	if o == nil {
		return nil, false
	}
	return o.Owner.Get(), o.Owner.IsSet()
}

// HasOwner returns a boolean if a field has been set.
func (o *CoolingFeed) HasOwner() bool {
	if o != nil && o.Owner.IsSet() {
		return true
	}

	return false
}

// SetOwner gets a reference to the given NullableBriefOwner and assigns it to the Owner field.
func (o *CoolingFeed) SetOwner(v BriefOwner) {
	o.Owner.Set(&v)
}

// SetOwnerNil sets the value for Owner to be an explicit nil
func (o *CoolingFeed) SetOwnerNil() {
	o.Owner.Set(nil)
}

// UnsetOwner ensures that no value is present for Owner, not even an explicit nil
func (o *CoolingFeed) UnsetOwner() {
	o.Owner.Unset()
}

// GetComments returns the Comments field value if set, zero value otherwise.
func (o *CoolingFeed) GetComments() string {
	if o == nil || IsNil(o.Comments) {
		var ret string
		return ret
	}
	return *o.Comments
}

// GetCommentsOk returns a tuple with the Comments field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CoolingFeed) GetCommentsOk() (*string, bool) {
	if o == nil || IsNil(o.Comments) {
		return nil, false
	}
	return o.Comments, true
}

// HasComments returns a boolean if a field has been set.
func (o *CoolingFeed) HasComments() bool {
	if o != nil && !IsNil(o.Comments) {
		return true
	}

	return false
}

// SetComments gets a reference to the given string and assigns it to the Comments field.
func (o *CoolingFeed) SetComments(v string) {
	o.Comments = &v
}

// GetTags returns the Tags field value if set, zero value otherwise.
func (o *CoolingFeed) GetTags() []NestedTag {
	if o == nil || IsNil(o.Tags) {
		var ret []NestedTag
		return ret
	}
	return o.Tags
}

// GetTagsOk returns a tuple with the Tags field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CoolingFeed) GetTagsOk() ([]NestedTag, bool) {
	if o == nil || IsNil(o.Tags) {
		return nil, false
	}
	return o.Tags, true
}

// HasTags returns a boolean if a field has been set.
func (o *CoolingFeed) HasTags() bool {
	if o != nil && !IsNil(o.Tags) {
		return true
	}

	return false
}

// SetTags gets a reference to the given []NestedTag and assigns it to the Tags field.
func (o *CoolingFeed) SetTags(v []NestedTag) {
	o.Tags = v
}

// GetCustomFields returns the CustomFields field value if set, zero value otherwise.
func (o *CoolingFeed) GetCustomFields() map[string]interface{} {
	if o == nil || IsNil(o.CustomFields) {
		var ret map[string]interface{}
		return ret
	}
	return o.CustomFields
}

// GetCustomFieldsOk returns a tuple with the CustomFields field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *CoolingFeed) GetCustomFieldsOk() (map[string]interface{}, bool) {
	if o == nil || IsNil(o.CustomFields) {
		return map[string]interface{}{}, false
	}
	return o.CustomFields, true
}

// HasCustomFields returns a boolean if a field has been set.
func (o *CoolingFeed) HasCustomFields() bool {
	if o != nil && !IsNil(o.CustomFields) {
		return true
	}

	return false
}

// SetCustomFields gets a reference to the given map[string]interface{} and assigns it to the CustomFields field.
func (o *CoolingFeed) SetCustomFields(v map[string]interface{}) {
	o.CustomFields = v
}

// GetCreated returns the Created field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *CoolingFeed) GetCreated() time.Time {
	if o == nil || IsNil(o.Created.Get()) {
		var ret time.Time
		return ret
	}
	return *o.Created.Get()
}

// GetCreatedOk returns a tuple with the Created field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *CoolingFeed) GetCreatedOk() (*time.Time, bool) {
	if o == nil {
		return nil, false
	}
	return o.Created.Get(), o.Created.IsSet()
}

// HasCreated returns a boolean if a field has been set.
func (o *CoolingFeed) HasCreated() bool {
	if o != nil && o.Created.IsSet() {
		return true
	}

	return false
}

// SetCreated gets a reference to the given NullableTime and assigns it to the Created field.
func (o *CoolingFeed) SetCreated(v time.Time) {
	o.Created.Set(&v)
}

// SetCreatedNil sets the value for Created to be an explicit nil
func (o *CoolingFeed) SetCreatedNil() {
	o.Created.Set(nil)
}

// UnsetCreated ensures that no value is present for Created, not even an explicit nil
func (o *CoolingFeed) UnsetCreated() {
	o.Created.Unset()
}

// GetLastUpdated returns the LastUpdated field value if set, zero value otherwise (both if not set or set to explicit null).
func (o *CoolingFeed) GetLastUpdated() time.Time {
	if o == nil || IsNil(o.LastUpdated.Get()) {
		var ret time.Time
		return ret
	}
	return *o.LastUpdated.Get()
}

// GetLastUpdatedOk returns a tuple with the LastUpdated field value if set, nil otherwise
// and a boolean to check if the value has been set.
// NOTE: If the value is an explicit nil, `nil, true` will be returned
func (o *CoolingFeed) GetLastUpdatedOk() (*time.Time, bool) {
	if o == nil {
		return nil, false
	}
	return o.LastUpdated.Get(), o.LastUpdated.IsSet()
}

// HasLastUpdated returns a boolean if a field has been set.
func (o *CoolingFeed) HasLastUpdated() bool {
	if o != nil && o.LastUpdated.IsSet() {
		return true
	}

	return false
}

// SetLastUpdated gets a reference to the given NullableTime and assigns it to the LastUpdated field.
func (o *CoolingFeed) SetLastUpdated(v time.Time) {
	o.LastUpdated.Set(&v)
}

// SetLastUpdatedNil sets the value for LastUpdated to be an explicit nil
func (o *CoolingFeed) SetLastUpdatedNil() {
	o.LastUpdated.Set(nil)
}

// UnsetLastUpdated ensures that no value is present for LastUpdated, not even an explicit nil
func (o *CoolingFeed) UnsetLastUpdated() {
	o.LastUpdated.Unset()
}

func (o CoolingFeed) MarshalJSON() ([]byte, error) {
	toSerialize, err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o CoolingFeed) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["id"] = o.Id
	if !IsNil(o.Url) {
		toSerialize["url"] = o.Url
	}
	if !IsNil(o.DisplayUrl) {
		toSerialize["display_url"] = o.DisplayUrl
	}
	if !IsNil(o.Display) {
		toSerialize["display"] = o.Display
	}
	if !IsNil(o.CoolingSource) {
		toSerialize["cooling_source"] = o.CoolingSource
	}
	if o.Rack.IsSet() {
		toSerialize["rack"] = o.Rack.Get()
	}
	if !IsNil(o.Name) {
		toSerialize["name"] = o.Name
	}
	if !IsNil(o.Status) {
		toSerialize["status"] = o.Status
	}
	if o.CoolingCapacity.IsSet() {
		toSerialize["cooling_capacity"] = o.CoolingCapacity.Get()
	}
	if o.MaxFlow.IsSet() {
		toSerialize["max_flow"] = o.MaxFlow.Get()
	}
	if o.MaxFlowUnit.IsSet() {
		toSerialize["max_flow_unit"] = o.MaxFlowUnit.Get()
	}
	if !IsNil(o.Description) {
		toSerialize["description"] = o.Description
	}
	if o.Tenant.IsSet() {
		toSerialize["tenant"] = o.Tenant.Get()
	}
	if o.Owner.IsSet() {
		toSerialize["owner"] = o.Owner.Get()
	}
	if !IsNil(o.Comments) {
		toSerialize["comments"] = o.Comments
	}
	if !IsNil(o.Tags) {
		toSerialize["tags"] = o.Tags
	}
	if !IsNil(o.CustomFields) {
		toSerialize["custom_fields"] = o.CustomFields
	}
	if o.Created.IsSet() {
		toSerialize["created"] = o.Created.Get()
	}
	if o.LastUpdated.IsSet() {
		toSerialize["last_updated"] = o.LastUpdated.Get()
	}

	for key, value := range o.AdditionalProperties {
		toSerialize[key] = value
	}

	return toSerialize, nil
}

func (o *CoolingFeed) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"id",
	}

	// defaultValueFuncMap captures the default values for required properties.
	// These values are used when required properties are missing from the payload.
	defaultValueFuncMap := map[string]func() interface{}{}
	var defaultValueApplied bool
	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err
	}

	for _, requiredProperty := range requiredProperties {
		if _, exists := allProperties[requiredProperty]; !exists {
			if _, ok := defaultValueFuncMap[requiredProperty]; ok {
				allProperties[requiredProperty] = defaultValueFuncMap[requiredProperty]()
				defaultValueApplied = true
			}
		}
		if value, exists := allProperties[requiredProperty]; !exists || value == "" {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	if defaultValueApplied {
		data, err = json.Marshal(allProperties)
		if err != nil {
			return err
		}
	}
	varCoolingFeed := _CoolingFeed{}

	err = json.Unmarshal(data, &varCoolingFeed)

	if err != nil {
		return err
	}

	*o = CoolingFeed(varCoolingFeed)

	additionalProperties := make(map[string]interface{})

	if err = json.Unmarshal(data, &additionalProperties); err == nil {
		delete(additionalProperties, "id")
		delete(additionalProperties, "url")
		delete(additionalProperties, "display_url")
		delete(additionalProperties, "display")
		delete(additionalProperties, "cooling_source")
		delete(additionalProperties, "rack")
		delete(additionalProperties, "name")
		delete(additionalProperties, "status")
		delete(additionalProperties, "cooling_capacity")
		delete(additionalProperties, "max_flow")
		delete(additionalProperties, "max_flow_unit")
		delete(additionalProperties, "description")
		delete(additionalProperties, "tenant")
		delete(additionalProperties, "owner")
		delete(additionalProperties, "comments")
		delete(additionalProperties, "tags")
		delete(additionalProperties, "custom_fields")
		delete(additionalProperties, "created")
		delete(additionalProperties, "last_updated")
		o.AdditionalProperties = additionalProperties
	}

	return err
}

type NullableCoolingFeed struct {
	value *CoolingFeed
	isSet bool
}

func (v NullableCoolingFeed) Get() *CoolingFeed {
	return v.value
}

func (v *NullableCoolingFeed) Set(val *CoolingFeed) {
	v.value = val
	v.isSet = true
}

func (v NullableCoolingFeed) IsSet() bool {
	return v.isSet
}

func (v *NullableCoolingFeed) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableCoolingFeed(val *CoolingFeed) *NullableCoolingFeed {
	return &NullableCoolingFeed{value: val, isSet: true}
}

func (v NullableCoolingFeed) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableCoolingFeed) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}
